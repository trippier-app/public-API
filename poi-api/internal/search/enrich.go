...
// enrichRaw runs every registered enricher over raw in place, skipping
// failures silently, and returns the enriched POI slice.
...
// applyEnricher enriches each in-radius target POI in raw using enricher e
// over provider p's sources, skipping targets from the enricher's own provider.
