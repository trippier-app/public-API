...
type Service struct {
	...
	// need no eviction at realistic scale.
...
// Search runs the full POI pipeline for query q under ctx with geo-aware
// provider auto-selection: an explicit ?providers= list is used as-is,
// otherwise providers are chosen by country + category scores.
...
// q.CountryHint, q.ExcludeProviders and q.ProviderWeights overrides.
...
// the radius to the maximum MinRadius declared by any selected provider.
...
// ProvidersRecommend scores registry providers for (lat, lng) and
// requestedTypes yielding kind, keeping at most limit entries (0 = unlimited).
// It returns the recommendations sorted by composite score descending.
...
// types (using ctx for geocoding/country detection), restricted to those
...
// selectByCountry scores and filters registry providers for country cc by
// requestedTypes, applying weightOverrides and exclude, restricted to kind;
// the no-override path is memoised. It returns provider IDs, best score first.
...
// makeSelectionKey builds the stable selectByCountry cache key from cc,
// requestedTypes (sorted so caller order doesn't fragment the cache), and
// kind. It returns the resulting cache key.
...
// ParseProviderWeights deserialises raw, the JSON-encoded "provider_weights"
// query param; all values must be in [0, 1]. It returns the parsed weights,
// or an error if raw is malformed or out of range.
