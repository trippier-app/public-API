...
// CountryCode resolves (lat, lng) to an uppercase ISO 3166-1 alpha-2 country
// code via the Nominatim /reverse API, using ctx. It returns the code, or an
// error if the request fails or no result is found.
...
// ReverseGeocode resolves (lat, lng) to a city name via the Nominatim OSM API,
// using ctx. It returns the most specific populated place name (city > town >
// village > county), or an error if the request fails or none is found.
