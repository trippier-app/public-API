...
type Meta struct {
	...
	// Priority drives dedup tie-breaks: when several providers contribute to the same
	// merged POI, the highest Priority is "primary" (canonical ID, name, images).
	// Higher = preferred; zero is fine when the tie direction doesn't matter.
	Priority int
	// AccuracyMeters is the provider's typical coordinate precision, used by the
	// dedup proximity threshold and Wikipedia-enrichment radius so a coarse provider
	// isn't discarded for known-imprecise coords. Zero means "use the global default".
