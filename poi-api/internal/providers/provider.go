...
// NormalizeLang lowercases and validates a user-supplied language code so it is
// safe to interpolate into a Wikimedia subdomain (guarding against SSRF). It
// returns the normalized code, or "" when lang is empty or malformed.
...
// (nil) silently and stopping on the first error. It returns the built
// providers, or an error from the first factory that failed.
...
// CommonsFileURL builds the canonical Special:FilePath URL for a Wikimedia
// Commons reference such as "File:Eiffel_Tower.jpg". It returns "" when ref is
// empty, a category, or otherwise unusable.
...
// SafeURL filters s, an untrusted upstream URL, down to allow-listed schemes. It
// returns s if it starts with http://, https://, or mailto:, else "" — defanging
// javascript: and other XSS-prone schemes.
