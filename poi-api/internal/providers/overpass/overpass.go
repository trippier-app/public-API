...
// overpassResponse is the top-level Overpass API JSON response.
type overpassResponse struct {
	Elements []overpassElement `json:"elements"`
}

// overpassElement is a single node, way, or relation returned by Overpass.
...
// overpassCenter holds the computed centroid of a way or relation.
...
// buildStatements builds Overpass node and way statements for query q using the
// given filters, kept separate so buildQuery can apply independent output
// limits. It returns the node and way statements.
