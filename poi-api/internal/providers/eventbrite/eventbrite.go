...
// Search queries Eventbrite for events near q, using a BYOK token read from ctx.
// The radius is clamped to [minRadiusKm, maxRadiusKm] km. It returns the matching
// raw POIs, or nil, nil if no token is present in ctx.
