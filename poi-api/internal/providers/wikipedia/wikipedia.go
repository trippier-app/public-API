...
// base holds the shared HTTP client and endpoints for the Wikipedia adapters.
type base struct {
	client  *http.Client
	baseURL string
...
// geosearchPage is a single page result from the Wikipedia Geosearch API.
...
// enrichedPage is a geosearch page augmented with extract, thumbnail, and Wikidata ID.
...
// Search implements providers.Provider. It is not in AllProviders; it is used
// explicitly (enrichment, tests). It searches near the coordinates and radius
// in q using ctx, returning matching POIs, or an error.
...
// Search implements providers.Provider. It searches near the coordinates and
// radius in q using ctx, returning only articles classified as cultural
// festivals in Wikidata (those without a Wikidata ID are dropped), or an error.
